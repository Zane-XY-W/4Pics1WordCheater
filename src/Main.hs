...
argsP :: Args
argsP = Args { length = def &= name "n" &= typ "DIGIT" &= help "the length of the word (2-8)"
             , letters = def &= name "l" &= typ "ALPHABET" &= help "the letters used to guess (a-z) case-insensitive"
...
showHelp :: IO ()
showHelp = print (helpText [] HelpFormatDefault argsD)

printResults :: Args -> IO ()
printResults (Args (Just n) (Just l)) = if (n `elem` map show [2 :: Int .. 8]) && all isAlpha l
